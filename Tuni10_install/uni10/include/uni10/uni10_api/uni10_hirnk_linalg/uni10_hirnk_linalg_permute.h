#ifndef __UNI10_HIGH_RANK_LINALG_PERMUTE_H__
#define __UNI10_HIGH_RANK_LINALG_PERMUTE_H__
...
namespace uni10{
  ...
  template<typename uni10_type>
    UniTensor<uni10_type> permute( const UniTensor<uni10_type>& T, uni10_int32* newLabels, uni10_int32 rowBondNum);

  template<typename uni10_type>
    UniTensor<uni10_type> permute( const UniTensor<uni10_type>& T, uni10_int32 rowBondNum);
  ...
  template<typename uni10_type>
    UniTensor<uni10_type> permute( const UniTensor<uni10_type>& T, const std::vector<uni10_int32>& newLabels, uni10_int32 rowBondNum){
      ...
      if(inorder && (*T.RBondNum) == rowBondNum) {
        ...
        // The version of on one node gpu is Developping
